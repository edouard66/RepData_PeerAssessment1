...
According to the experience, the subject has a peak time in the interval at `r top.interval`h.
The average number of steps from the user during this interval is `r max.steps`.
...
2. Let's replace those NAs with the daily mean calculated earlier. To do so, let's divide the mean.day by the number of intervals in one day (288). The result is an interval mean of `r mean.interval` steps per interval.
...
```
4. Based on this new dataset, the mean number of steps per day is `r mean.day.new`. Compared with the initial value, the mean has increased. However, the median value hasn't changed at `r median.day.new`.
